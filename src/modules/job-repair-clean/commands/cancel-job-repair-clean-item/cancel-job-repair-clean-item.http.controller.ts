...
import { CancelJobRepairCleanItemServiceResult } from './cancel-job-repair-clean-item.service';
...
import { CancelJobRepairCleanItemRequestDto } from './cancel-job-repair-clean-item.request.dto';
import { VendorNotFoundError } from '@src/modules/vendor/domain/vendor.error';

@Controller(routesV1.version)
export class CancelJobRepairCleanItemHttpController {
  constructor(
  ...
  @ApiOperation({ summary: 'cancel job repair clean item' })
  @ApiBearerAuth()
  @ApiParam({
    name: 'id',
  ...
  @Post(routesV1.jobRepairClean.cancel)
  async create(
    @ReqUser() user: RequestUser,
    @Param('id') estimateDetailId: bigint,
    @Body() body: CancelJobRepairCleanItemRequestDto,
  ): Promise<JobRepairCleanResponseDto> {
    const command = new CancelJobRepairCleanItemCommand({
      estimateDetailId,
      ...body,
    ...
    const result: CancelJobRepairCleanItemServiceResult =
