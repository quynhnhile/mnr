...
import { UpdateOperationRequestDto } from './update-tariff-group.request.dto';
import { UpdateOperaionServiceResult } from './update-tariff-group.service';
...
export class UpdateOperationHttpController {
  ...
  @ApiResponse({
    ...
    description: `${TariffGroupNotFoundError.message} | ${OperationNotFoundError.message}`,
  ...
  @Put(routesV1.tariffGroup.updateOperation)
  async update(
    ...
    const result: UpdateOperaionServiceResult = await this.commandBus.execute(
      command,
    );
