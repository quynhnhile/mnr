...
import {
  AllEstimateDetailAlreadyCanceledError,
...
export type CancelEstimateServiceResult = Result<
  EstimateEntity,
  | EstimateNotFoundError
...
@CommandHandler(CancelEstimateCommand)
export class CancelEstimateService implements ICommandHandler {
  constructor(
  ...
  async execute(
    command: CancelEstimateCommand,
  ): Promise<CancelEstimateServiceResult> {
    ...
    const props = command.getExtendedProps<CancelEstimateCommand>();
    ...
    const allEstimateDetails = foundEstimateDetails
      .filter((item) => item.cancelBy == null && item.cancelBy == null)
      .map((item) => {
        return item;
      });
    if (allEstimateDetails.length == 0) {
      return Err(new AllEstimateDetailAlreadyCanceledError());
    }
    ...
    const cancelResult = estimate.cancel(props);
