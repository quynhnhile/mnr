...
export class Agent {
  ...
  public updateRequest({
    ...
    return async (query: any = {}, payload: any = {}): Promise<any> => {
      const baseParams = this.getBaseParams?.() ?? {};
  ...
  async requestWithParams({
    ...
    const pathTemplate = UrlTemplate.parse(newPath);
  ...
  transformKey(payload: unknown, keyMapping: Record<string, string>): unknown {
    if (!payload) {
      return;
    }
